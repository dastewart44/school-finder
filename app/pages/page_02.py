...
def grade():
    ...
    dems = df_avgs_selected[['start_gpa_avg', 'end_gpa_avg']].iloc[0] * 100
    colors = ['lightgrey'] * len(dems)
    y=df_avgs_selected['school_id'].iloc[0] * 100
    # Create a figure and axes

    race_categories = ['Asian', 'Black or African American', 'Hispanic or Latino', 'White', 'Other']
    race_percentages = df_avgs_selected[['pct_asian', 'pct_black', 'pct_hispanic', 'pct_white', 'pct_other']].iloc[0] * 100

    fig = go.Figure(data=[go.Pie(labels=race_categories, values=race_percentages)])
...
def main():
    ...
    col1, col2, col3, col4 = st.columns((3, .25, 6, .25))
